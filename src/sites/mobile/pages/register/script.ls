...
$ ->
	...
	$ '#screen-name > input' .keyup ->
		...
		if sn != ''
			err = switch
				| not sn.match /^[a-zA-Z0-9\-]+$/ => LOCALE.sites.mobile.pages._register.screen_name_error_1
				| sn.length < 3chars              => LOCALE.sites.mobile.pages._register.screen_name_error_2
				| sn.length > 20chars             => LOCALE.sites.mobile.pages._register.screen_name_error_3
				| _                               => null
			...
			if err
				...
				$ '#screen-name > .info'
					...
					..children \span .text LOCALE.sites.mobile.pages._register.screen_name_info_1
				...
				$.ajax "#{CONFIG.web-api-url}/screenname/available" {
					...
					if result.available
						$ '#screen-name > .info'
							...
							..children \span .text LOCALE.sites.mobile.pages._register.screen_name_info_2
							..attr \data-state \ok
					else
						$ '#screen-name > .info'
							..children \i .attr \class 'fa fa-exclamation-triangle'
							..children \span .text LOCALE.sites.mobile.pages._register.screen_name_info_3
							..attr \data-state \error
				.fail (err) ->
					$ '#screen-name > .info'
						..children \i .attr \class 'fa fa-exclamation-triangle'
						..children \span .text LOCALE.sites.mobile.pages._register.screen_name_info_4
						..attr \data-state \error
		else
			$ '#screen-name > .profile-page-url-preview' .text ""
	...
	$ '#password > input' .keyup ->
		...
		if password != ''
			err = switch
				| password.length < 8chars       => LOCALE.sites.mobile.pages._register.password_error_1
				| _                              => null
			if err
				$ '#password > .info'
					..children \i .attr \class 'fa fa-exclamation-triangle'
				...
				$ '#password > .info'
					..children \i .attr \class 'fa fa-check'
					..children \span .text LOCALE.sites.mobile.pages._register.password_info_1
					..attr \data-state \ok
	...
	$ '#retype-password > input' .keyup ->
		...
		if retyped-password != ''
			err = switch
				| retyped-password != password   => LOCALE.sites.mobile.pages._register.retype_password_error_1
				| _                              => null
			if err
				...
				$ '#retype-password > .info'
					..children \i .attr \class 'fa fa-check'
					..children \span .text LOCALE.sites.mobile.pages._register.retype_password_info_1
					..attr \data-state \ok
	...
	$form.submit (event) ->
		...
		$submit-button = $form.find '[type=submit]'
			..attr \disabled on
			..find \span .text LOCALE.sites.mobile.pages._register.creating
		...
		.fail ->
			$submit-button
				..attr \disabled off
				.find \span .text LOCALE.sites.mobile.pages._register.create
